...
\paragraph{依存パッケージ}
\begin{itemize}
\item \Pkg{bxoolbox} パッケージ
  \footnote{https://github.com/zr-tex8r/BXjatool （暫定）}
\item \Pkg{everyhook} パッケージ
\item \Pkg{svn-proc} パッケージ
\end{itemize}
...
\item |\uselineheadlineenindent|／|\nouselineheadlineenindent|\Means
  強制改行後の行頭での開き括弧調整を有効／無効にする。
  \Note 折り返しの行頭の調整には対応しない
  （これにはJFMの修正が必須）。
  現在(u)pTeXで標準的に用いられるJFMの場合、
